...
import pickle
import cv2
from cluster_feature import generate_clip_features
...
from base_network import RegNetwork
...
class ControlLDM(LatentDiffusion):
    ...
    def __init__(self, control_stage_config, control_key, only_mid_control, *args, **kwargs):
        ...
        if os.path.exists('models/Reflection_reg.pth'):
            state_dict = torch.load('models/Reflection_reg.pth')['net']
    ...
    def apply_model(self, x_noisy, t, cond, get_stable_features=True, batch=None, *args, **kwargs): # TODO
        ...
        if cond['c_concat'] is None:
            ...
            bbx_region_reflection = self.gen_bbx(pred_t, fg_instance_bbx, bbx_mask)
            hint = torch.cat(cond['c_concat'], 1)
            hint_6 = torch.cat((hint, bbx_region_reflection), 1)
            control = self.control_model(x=x_noisy, hint=hint_6, timesteps=t, context=cond_txt) 
            control = [c * scale for c, scale in zip(control, self.control_scales)]
            mid_control = control.pop()
            down_control = tuple(control)
            
            reflection_features = generate_clip_features(batch['cls'], batch['objectmask'])
